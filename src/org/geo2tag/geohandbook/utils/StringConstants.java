/**
 * 
 */
package org.geo2tag.geohandbook.utils;

/**
 * @author Mark Zaslavskiy
 *
 */
public class StringConstants {

	public static final String BACK_COMMAND = "Back";
	public static final String OK_COMMAND = "Ok";
	public static final String REFRESH_COMMAND = "Refresh";
	public static final String ABOUT_COMMAND = "About";
	public static final String HELP_COMMAND = "Help";
	public static final String EXIT_COMMAND = "Exit";
	public static final String CHANNELS_COMMAND = "Channels";
	
	public static final String ABOUT_HEADER = "About";
	public static final String ABOUT_TEXT = "GeoHandBook v0.1 (c) OSLL, 2013";
	public static final String HELP_HEADER = "Help";
	public static final String HELP_TEXT = "This application allows to get access for geo-related databases.";
	
	public static final String USER_NAME_TEXT_FIELD = "UserName";
	public static final String PASSWORD_TEXT_FIELD = "Password";

	public static final String DEFAULT_USER_NAME = "Mark";
	public static final String DEFAULT_PASSWORD = "test";
	public static final String HANDBOOK_NAME = "default";
	public static final String CHANNELS = "Channels";
}
